...
def load_model(path,path_text,name,col_list,i,quarter,prodfile,countryfile):
  ...
  for word in l2:
    df=df[df.word != word]
  location = F"results/nerresults/company/{name}"
  df.to_csv(location, index = False, header=True) 
  rep= pd.read_csv('csvfiles//replace - Sheet1.csv', usecols=col_list)
#product list
  ...
  if(i==0):
    ...
    new_prod.insert(0,'quarter',quarter)
    new_prod.to_csv(r'results/nerresults//prod2.csv', index =False, header=True)
    data = pd.read_csv(prodfile) 
    data2 = pd.read_csv("results/nerresults//prod2.csv")    
...
#country list
  ...
  if(i==0):
    ...
    new_con.insert(0,'quarter',quarter)
    new_con.to_csv(r'results/nerresults//country2.csv', index =False, header=True)
    data = pd.read_csv(countryfile) 
    data2 = pd.read_csv("results/nerresults//country2.csv")    
    for col in data2.columns: 
      if col not in data.columns:
        data[col] = np.nan
      data.at[20,col] = data2[col].values[0]
...
def nercall(model_save_name,col_list,col_rep_list,prodfile,countryfile,pngname,mp4name,pc,transcript):
  path=F"models/{model_save_name}"
  mp4name=os.path.join('results','nerresults',mp4name)
  filelist= pd.read_csv('csvfiles//path.csv', usecols=col_list)
...
# Scatter plot for country 
  ...
  location=F"results/nerresults/{pngname}"
...
#bar_chart_race on products
  ...
  df_prod=df_prod[:-1]
  df_prod.to_csv(r'results/nerresults//prod_plot.csv', index =False, header=True)
  df = pd.read_csv('results/nerresults//prod_plot.csv', index_col='quarter', 
                  parse_dates=['quarter'])
  bcr.bar_chart_race(df=df, steps_per_period=30, fixed_order=True, period_length=8000,orientation='v',filename=mp4name)

  
  #Specific Quarter
  df = pd.read_csv(prodfile, index_col='quarter', 
                  parse_dates=['quarter'])
  
  d=pd.DataFrame(df.loc[str(transcript)])
  d.drop(d[d[str(transcript)] <2].index, inplace = True) 
  s=d[str(transcript)]
  ...
  plt.savefig('results//nerresults//quarterprod.png')
  plt.show()
  os.remove('results//nerresults//prod2.csv')
  os.remove('results//nerresults//country2.csv')
  os.remove(prodfile)
  os.remove(countryfile)
